...
$Route->add('/dashboard/sales/orders', function () {
    ...
    $Template->assign("menukey", "orders");

    $accid = $Template->storage("accid");
    $root = $Core->UserInfo($accid, "root");

    $Template->render("dashboard.{$root}.orders");
}, 'GET');
...
$Route->add('/dashboard/sales/sell', function () {
    ...
    $Template->assign("menukey", "sell item");

    $accid = $Template->storage("accid");
    $root = $Core->UserInfo($accid, "root");

    $Template->render("dashboard.{$root}.sell");
}, 'GET');
